  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 700 }}>Wallet & Credits</h2>
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', color: 'var(--primary)', backgroundColor: 'rgba(16, 185, 129, 0.1)', padding: '0.5rem 1rem', borderRadius: '8px', fontSize: '0.875rem', fontWeight: 600 }}>
          <ShieldCheck size={16} /> Secure Payments Enabled
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
        ...
        <div className="glass-card" style={{ 
          ...
          <div>
            <div style={{ color: 'var(--text-secondary)', fontSize: '1rem', marginBottom: '0.5rem' }}>Total Available Balance</div>
          ...
          <div style={{ marginTop: '3rem', display: 'flex', gap: '1rem' }}>
            <button className="btn-primary" style={{ flex: 1, padding: '1rem' }}>Top Up Now</button>
            <button className="glass-card" style={{ flex: 1, padding: '1rem', fontWeight: 600 }}>Transaction Logs</button>
          </div>
        </div>

        {/* Payment Methods */}
        <div className="glass-card" style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <h3 style={{ fontSize: '1.125rem', fontWeight: 600 }}>Add Credits</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div className="glass-card" style={{ padding: '1.5rem', cursor: 'pointer', border: '1px solid var(--primary)' }}>
              ...
              <div style={{ fontWeight: 600, fontSize: '1rem' }}>Stripe / Visa</div>
              <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Instant USD Top-up</div>
            </div>
            <div className="glass-card" style={{ padding: '1.5rem', cursor: 'pointer' }}>
              ...
              <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Crypto Payment Gateway</div>
            </div>
          </div>
          <div style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Info size={14} /> Currency exchange rates are updated every 5 minutes.
          </div>
        </div>
      </div>
          ...
          {transactions.map((tx) => (
            <div key={tx.id} style={{ padding: '1.25rem', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontWeight: 700, color: tx.amount >= 0 ? 'var(--primary)' : 'var(--text-main)' }}>
                  {tx.amount >= 0 ? '+' : ''}${Math.abs(tx.amount).toFixed(2)}
                </div>
                <div style={{ fontSize: '0.75rem', color: 'var(--primary)' }}>COMPLETED</div>
              </div>
            </div>
          ))}
